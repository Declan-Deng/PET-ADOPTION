...
export const UserProvider = ({ children }) => {
      ...
      if (token && userData) {
        ...
        const userDataWithProfile = {
          ...
          profile: {
            ...
            avatar: parsedUserData.profile?.avatar || null,
          },
        };
  ...
  const updateProfile = async (updatedProfile) => {
    try {
      ...
      if (
        ...
        // TODO: 这里应该添加上传头像到服务器的逻辑
        // 暂时直接使用本地URI
        processedAvatar = updatedProfile.avatar;
      }
      ...
      if (!responseData.user) {
        console.error("服务器返回的数据不完整:", responseData);
      ...
      const userDataWithProfile = {
        ...
        profile: {
          ...
          avatar: processedAvatar || responseData.user.profile?.avatar,
        },
      };

      console.log("更新后的用户数据:", userDataWithProfile);
  ...
  const addAdoption = async (adoptionData) => {
    try {
      ...
      const updatedUser = {
        ...user,
        adoptions: [...(user.adoptions || []), adoptionData],
      };
