...
import { ENDPOINTS, getApiHeaders, fetchWithRetry } from "../config/api";
...
export const UserProvider = ({ children }) => {
  ...
  const login = async (username, password) => {
    try {
      const response = await fetchWithRetry(ENDPOINTS.LOGIN, {
        method: "POST",
        headers: {
  ...
  const register = async (userData) => {
    try {
      const response = await fetchWithRetry(ENDPOINTS.REGISTER, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const updateProfile = async (updatedProfile) => {
    try {
      ...
      const response = await fetchWithRetry(ENDPOINTS.PROFILE, {
        method: "PUT",
  ...
  const uploadAvatar = async (imageUri) => {
    try {
      ...
      const response = await fetchWithRetry(ENDPOINTS.UPLOAD_AVATAR, {
        method: "POST",
        headers: {
  ...
  const addPublication = async (publication) => {
    try {
      const token = await AsyncStorage.getItem("userToken");
      const response = await fetchWithRetry(ENDPOINTS.PETS, {
        method: "POST",
        headers: getApiHeaders(token),
  ...
  const addAdoption = async (adoptionData) => {
    try {
      const token = await AsyncStorage.getItem("userToken");
      const response = await fetchWithRetry(ENDPOINTS.ADOPTIONS, {
        method: "POST",
        headers: getApiHeaders(token),
      ...
      // 获取最新的申请记录列表
      const getResponse = await fetchWithRetry(ENDPOINTS.ADOPTIONS, {
        headers: getApiHeaders(token),
      });
  ...
  const cancelAdoption = async (adoptionId) => {
    try {
      ...
      const response = await fetchWithRetry(
        ENDPOINTS.ADOPTION_DETAIL(adoptionId),
        {
          method: "DELETE",
          headers: getApiHeaders(token),
        }
