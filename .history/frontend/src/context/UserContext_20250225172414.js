...
export const UserProvider = ({ children }) => {
  ...
  const updateProfile = async (updatedProfile) => {
    try {
      ...
      if (
        ...
        console.log("准备上传头像...");
        // 上传头像
        const uploadResponse = await fetch(API_ENDPOINTS.UPLOAD_AVATAR, {
          method: "POST",
          headers: {
  ...
  const addAdoption = async (adoptionData) => {
    try {
      ...
      // 先提交新的申请
      const submitResponse = await fetch(API_ENDPOINTS.ADOPTIONS, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      // 获取最新的申请记录列表
      const getResponse = await fetch(API_ENDPOINTS.ADOPTIONS, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const cancelAdoption = async (adoptionId) => {
    try {
      ...
      // 发送取消请求
      const response = await fetch(API_ENDPOINTS.CANCEL_ADOPTION(adoptionId), {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
