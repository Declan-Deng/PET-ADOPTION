...
const PetDetailScreen = ({ route, navigation }) => {
  ...
  const userAdoption = React.useMemo(() => {
    if (!user?.adoptions || !Array.isArray(user.adoptions)) return null;
    return user.adoptions.find((adoption) => {
      // 检查 adoption 是否有效
      if (!adoption || typeof adoption !== "object") return false;
      // 检查 pet 字段
      const adoptionPetId = adoption.pet?._id || adoption.pet;
      return adoptionPetId === petId;
    });
  }, [user, petId]);
  ...
  const handleAdopt = async () => {
    ...
    // 检查是否是自己发布的宠物
    if (isOwner) {
      Alert.alert("提示", "不能申请领养自己发布的宠物");
      return;
    }

    // 检查是否已经申请过
    if (userAdoption) {
      Alert.alert("提示", "您已经申请过这只宠物了");
      return;
    }
