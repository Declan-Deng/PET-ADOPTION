...
const PublishScreen = ({ navigation }) => {
  ...
  const currentBreedList = React.useMemo(() => {
    if (!formData.petType || formData.petType === "other") return [];
  ...
  const handleBreedSelect = React.useCallback(
    (breed) => {
      updateField("breed", breed.label);
      setShowBreedSelector(false);
      if (breed.label.includes("其他")) {
          ...
          <Surface style={styles.surface} elevation={2}>
            ...
            <View style={styles.formSection}>
              ...
              {formData.petType === "other" ? (
                <>
                  <TouchableOpacity
                    onPress={() => setShowOtherPetTypeSelector(true)}
                    activeOpacity={0.7}
                  >
                    <View style={styles.pickerButton}>
                      <Text style={styles.pickerButtonText}>
                        {formData.otherType || "选择动物类型"}
                      </Text>
                ...
                <>
                  ...
                  <Portal>
                    ...
                    >
                      ...
                      <Dialog.Actions>
                        <Button onPress={() => setShowBreedSelector(false)}>
                          关闭
                        </Button>
                      </Dialog.Actions>
                    </Dialog>
