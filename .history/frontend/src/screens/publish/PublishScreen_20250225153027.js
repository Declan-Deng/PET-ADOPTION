...
const PublishScreen = ({ navigation }) => {
  ...
  const [showOtherPetTypeDropDown, setShowOtherPetTypeDropDown] =
  ...
  // 将品种数据转换为DropDown需要的格式
  const breedDropDownList = React.useMemo(() => {
    return currentBreedList.map((item) => ({
      label: item.label,
      value: item.value,
    }));
  }, [currentBreedList]);

  // 将其他宠物类型数据转换为DropDown需要的格式
  const otherPetTypeDropDownList = React.useMemo(() => {
    return otherPetTypes.map((item) => ({
      label: item.label,
      value: item.value,
    }));
  }, []);
              ...
              {formData.petType === "other" ? (
                <>
                  <DropDown
                    label="动物类型"
                    mode="outlined"
                    visible={showOtherPetTypeDropDown}
                    showDropDown={() => setShowOtherPetTypeDropDown(true)}
                    onDismiss={() => setShowOtherPetTypeDropDown(false)}
                    value={formData.otherType}
                    setValue={(value) => updateField("otherType", value)}
                    list={otherPetTypeDropDownList}
                ...
                <>
                  <DropDown
                    ...
                    showDropDown={() => setShowBreedDropDown(true)}
                    onDismiss={() => setShowBreedDropDown(false)}
