...
const PetDetailScreen = ({ route, navigation }) => {
    ...
    try {
      ...
      if (routePet) {
        ...
        if (foundPet) {
          ...
          finalPetData = data.data || data; // 确保处理正确的数据结构
        }
      }
  ...
  const pet = React.useMemo(() => {
    ...
    console.log("实际的宠物数据:", actualPetData);
    ...
    // 如果是从路由传递过来的完整宠物数据，优先使用路由数据中的描述和要求
    if (routePet) {
      processedPet.description =
        routePet.description || processedPet.description;
      processedPet.requirements =
        routePet.requirements || processedPet.requirements;
      processedPet.medical = {
        ...processedPet.medical,
        ...routePet.medical,
      };
    }
    ...
    console.log("医疗信息:", processedPet.medical);
