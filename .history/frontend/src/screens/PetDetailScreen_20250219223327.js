...
const PetDetailScreen = ({ route, navigation }) => {
  ...
  const userAdoption = React.useMemo(() => {
    ...
    return user.adoptions.find(
      (adoption) => adoption.pet?._id === petId || adoption.petId === petId
    );
  }, [user, petId]);
  ...
  const fetchPetDetail = React.useCallback(async () => {
    try {
      ...
      if (routePet) {
        ...
        // 如果是从申请记录来的，需要使用 pet 字段
        if (routePet.pet) {
          setPetData(routePet.pet);
        } else {
          setPetData(routePet);
        }
        return;
  ...
  const pet = React.useMemo(() => {
    ...
    const processedPet = {
      ...petData,
      description: petData.description || "暂无描述",
      requirements: petData.requirements || "暂无特殊要求",
      medical: {
        vaccinated: petData.medical?.vaccinated || false,
        sterilized: petData.medical?.sterilized || false,
        healthStatus: petData.medical?.healthStatus || "暂无信息",
      },
      images: Array.isArray(petData.images) ? petData.images : [petData.image],
    };
  ...
  const handleCancelAdoption = async () => {
    try {
      ...
      const response = await fetch(
        `http://192.168.3.74:5001/api/adoptions/${userAdoption._id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
