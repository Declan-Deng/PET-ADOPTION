...
const PetDetailScreen = ({ route, navigation }) => {
  ...
  const { petList, user, addAdoption, setUser } = React.useContext(UserContext);
  ...
  const userAdoption = React.useMemo(() => {
    ...
    // 如果从路由参数中获取到申请信息，优先使用
    if (petData?.adoption) {
      return {
        _id: petData.adoption._id,
        status: petData.adoption.status,
        date: petData.adoption.date,
        reason: petData.adoption.reason,
      };
    }
    // 否则从用户的申请列表中查找
  ...
  const handleCancelAdoption = async () => {
    try {
      ...
      // 刷新用户数据
      const userResponse = await fetch("http://192.168.3.74:5001/api/auth/me", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Surface style={styles.infoContainer} elevation={2}>
        ...
        <List.Item
          title="健康状态"
          description={pet.medical?.healthStatus || "未知"}
        ...
        {canAdopt ? (
          ...
          <Button
            ...
            style={[styles.adoptButton, { backgroundColor: "#f44336" }]}
            loading={submitting}
          >
            取消申请
          </Button>
