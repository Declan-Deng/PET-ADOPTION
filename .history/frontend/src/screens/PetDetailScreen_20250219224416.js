...
const PetDetailScreen = ({ route, navigation }) => {
  ...
  const userAdoption = React.useMemo(() => {
    ...
    console.log("从用户记录中找到的申请：", adoption);
    return adoption;
  ...
  const canAdopt = React.useMemo(() => {
    console.log("判断是否可以申请：", {
      ...
      userAdoptionStatus: userAdoption?.status,
    });
    return (
      !isOwner &&
      !userAdoption &&
  ...
  const fetchPetDetail = React.useCallback(async () => {
    try {
      ...
      if (routePet) {
        ...
        finalPetData = routePet.pet || routePet;
      } else {
        // 如果没有完整对象，尝试从列表中查找
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Surface style={styles.infoContainer} elevation={2}>
        ...
        {isOwner && <Text style={styles.ownerText}>这是您发布的宠物</Text>}
      </Surface>
    </ScrollView>
  );
};
