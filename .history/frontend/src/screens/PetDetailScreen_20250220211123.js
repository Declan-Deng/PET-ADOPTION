...
const PetDetailScreen = ({ route, navigation }) => {
  ...
  const pet = React.useMemo(() => {
    ...
    // 确保images是一个数组
    const processedImages = Array.isArray(petData.images)
      ? petData.images.filter((img) => img && typeof img === "string")
      : [];
  ...
  return (
    <ScrollView style={styles.container}>
      <ImageCarousel images={pet?.images?.length > 0 ? pet.images : []} />
