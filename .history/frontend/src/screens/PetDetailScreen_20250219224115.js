...
const PetDetailScreen = ({ route, navigation }) => {
  ...
  const [adoptionData, setAdoptionData] = React.useState(routeAdoption);
  ...
  const handleCancelAdoption = async () => {
    try {
      ...
      const adoptionId = userAdoption?._id;
      if (!adoptionId) {
        throw new Error("未找到申请记录");
      }

      const response = await fetch(
        `http://192.168.3.74:5001/api/adoptions/${adoptionId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
