...
import { ENDPOINTS, getApiHeaders } from "../config/api";
...
const PetListScreen = ({ navigation, route }) => {
  ...
  const fetchPets = React.useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_URL}${ENDPOINTS.PETS}`, {
      ...
      if (response.ok && result.success) {
        if (Array.isArray(result.data)) {
          ...
          console.error("返回的数据不是数组:", result);
          setError("获取宠物列表失败：数据格式错误");
        }
      } else {
        console.error("获取宠物列表失败:", result.message);
        setError(result.message || "获取宠物列表失败");
      }
    } catch (error) {
