...
const PetDetailScreen = ({ route, navigation }) => {
  ...
  const [userAdoption, setUserAdoption] = React.useState(routeAdoption);

  // 检查是否是宠物主人
  const isOwner = React.useMemo(() => {
    if (!user || !petData) return false;
    return user._id === (petData.owner?._id || petData.owner?.id);
  }, [user, petData]);
  ...
  const canAdopt = React.useMemo(() => {
    ...
    // 已被领养的宠物不能申请
    if (petData.status === "adopted") return false;
    // 已经申请过的不能再申请
    if (userAdoption) return false;
    return true;
  }, [user, petData, isOwner, userAdoption]);

  // 检查用户是否已申请过该宠物
  const checkUserAdoption = React.useCallback(async () => {
    try {
      ...
      const adoption = data.find((a) => a.pet?._id === petId);
      if (adoption) {
        setUserAdoption(adoption);
      }
    } catch (error) {
      ...
      <Surface style={styles.infoContainer} elevation={2}>
        ...
        {canAdopt ? (
          ...
          userAdoption.status === "approved" ? (
            ...
            <Text style={styles.cancelledText}>申请已取消</Text>
          )
        ) : pet.status === "adopted" ? (
          <Text style={styles.adoptedText}>该宠物已被领养</Text>
        ) : null}
      </Surface>
    </ScrollView>
  );
