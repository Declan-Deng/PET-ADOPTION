...
const ProfileScreen = ({ navigation }) => {
  ...
  const refreshUserData = React.useCallback(async () => {
    ...
    try {
      ...
      console.log("本地存储的用户数据是否存在:", !!localUserData);
      ...
      if (!userData || !userData.profile) {
        console.error("无效的用户数据格式:", userData);
        throw new Error("获取到的用户数据格式不正确");
      }

      console.log("成功获取新的用户数据");
      ...
      if (setUser && typeof setUser === "function") {
        ...
        console.error("setUser不可用，无法更新Context");
      }
      ...
      // 尝试从本地获取数据作为备份
      try {
        const backupData = await AsyncStorage.getItem("userData");
        if (backupData) {
          const parsedBackupData = JSON.parse(backupData);
          if (setUser && typeof setUser === "function") {
            setUser(parsedBackupData);
          }
          Alert.alert("提示", "网络连接失败，已加载本地数据", [
            {
              text: "重试",
              onPress: () => {
        ...
        console.error("读取本地备份数据失败:", backupError);
      }
