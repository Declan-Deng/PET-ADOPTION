...
const MyPublicationsScreen = ({ navigation, route }) => {
  ...
  const handleCancelPublication = async (petId, petName) => {
    Alert.alert(
      "确认取消发布",
      `确定要取消发布 ${petName} 吗？取消后将无法恢复。`,
      [
          ...
          onPress: async () => {
            try {
              ...
              const response = await fetch(
                `http://192.168.3.74:5001/api/pets/${petId}/cancel`,
                {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer ${token}`,
              ...
              // 从列表中移除已取消的发布
              setPublications(publications.filter((pub) => pub._id !== petId));
        ...
        right={(props) => (
          <View style={styles.rightContent}>
            <View style={styles.chipContainer}>
              ...
              <Chip
                mode="outlined"
                onPress={() => handleCancelPublication(item._id, item.petName)}
