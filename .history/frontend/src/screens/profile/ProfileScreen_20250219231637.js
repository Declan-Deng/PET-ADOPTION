...
const ProfileScreen = ({ navigation }) => {
  ...
  const refreshUserData = React.useCallback(async () => {
    try {
      ...
      console.log("用户数据响应状态:", userResponse.status);

      if (!userResponse.ok) {
        throw new Error(`服务器响应错误: ${userResponse.status}`);
      }
      ...
      if (!adoptionsResponse.ok) {
        throw new Error(`获取申请记录失败: ${adoptionsResponse.status}`);
      }

      const adoptionsData = await adoptionsResponse.json();
      console.log("获取到的申请记录:", adoptionsData);
      ...
      if (!publicationsResponse.ok) {
        throw new Error(`获取发布记录失败: ${publicationsResponse.status}`);
      }

      const publicationsData = await publicationsResponse.json();
      console.log("获取到的发布记录:", publicationsData);
